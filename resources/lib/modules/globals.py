...
class GlobalVariables(object):
    ...
    def add_directory_item(self, name, **params):
        ...
        item = xbmcgui.ListItem(label=str(name), offscreen=True)
        ...
        if "unwatched_episodes" in menu_item:
            item.setProperty("UnWatchedEpisodes", menu_item["unwatched_episodes"])
        if "watched_episodes" in menu_item:
            item.setProperty("WatchedEpisodes", menu_item["watched_episodes"])
        ...
        if (
            ...
            item.setProperty(
                "UnWatchedEpisodes", menu_item.get("episode_count", 0)
            )
        if "episode_count" in menu_item:
            item.setProperty("TotalEpisodes", menu_item["episode_count"])
        if "season_count" in menu_item:
            item.setProperty("TotalSeasons", menu_item["season_count"])
        if (
                "percent_played" in menu_item
                and menu_item.get("percent_played") is not None
        ):
            if float(menu_item.get("percent_played", 0)) > 0:
                item.setProperty("percentplayed", menu_item["percent_played"])
        if "resume_time" in menu_item and menu_item.get("resume_time") is not None:
            if int(menu_item.get("resume_time", 0)) > 0:
                params["resume"] = menu_item["resume_time"]
                item.setProperty("resumetime", menu_item["resume_time"])
    ...
    def is_window_visible(window):
        return xbmc.getCondVisibility('Window.IsActive("{}")'.format(window))
    ...
    def _apply_listitem_properties(item, info):
        for i in listitem_properties:
            if isinstance(i[0], tuple):
                ...
                if value:
                    item.setProperty(i[1], value)
            elif i[0] in info:
                item.setProperty(i[1], info[i[0]])
